...
namespace OopFactory.X12.Validation
{
    ...
    {
        public static void Add997Transaction(this FunctionGroup group, IEnumerable<FunctionalGroupResponse> groupResponses, bool includeErrors = false)
        {
            int transactionId = 0;
            ...
            foreach (var groupResponse in groupResponses)
            {
                ...
                foreach (var response in groupResponse.TransactionSetResponses)
                {
                    ...
                    if (!includeErrors && response.AcknowledgmentCode == AcknowledgmentCodeEnum.E_Accepted_ButErrorsWereNoted)
                    {
                        ak5.AK501_TransactionSetAcknowledgmentCode = AcknowledgmentCodeEnum.A_Accepted.ToString().Substring(0, 1);
                    } else {
